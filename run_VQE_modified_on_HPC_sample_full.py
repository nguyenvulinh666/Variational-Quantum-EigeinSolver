...
def main(params):
    ...
    if str(optimize.__name__)[:16] == 'Customize_QNSPSA':
        file_name_fubini_matrix_previous = f'{str(optimize.__name__)} - LR {eta} - shots {shots} - interation {interation} - {ansatz.name}({num_qubits},{reps}) - {entanglement} - J{J}h{h} - PreviousFubiniMatrix - {run_time+1}'
