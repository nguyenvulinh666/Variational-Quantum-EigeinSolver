...
def mini_derivate(param):
    theta_position, h, divide, initial_point, operator, learning_rate, ansatz, shots, sampler = param
    ...
    result = (learning_rate)*(Transverse_Ising_Measurement(operator, ansatz.bind_parameters({theta: plus_parameter[i] for i, theta in enumerate(ansatz.parameters)}), shots, sampler) - Transverse_Ising_Measurement(operator, ansatz.bind_parameters({theta: minus_parameter[i] for i, theta in enumerate(ansatz.parameters)}), shots, sampler))/(2*divide)
    return result
...
def Customize_Finite_Difference(operator, initial_point, learning_rate, ansatz, interation, shots, callback, sampler):
    ...
    for i in range(interation):
        ...
        for i in range(ansatz.num_parameters):
            params.append((i, h, h, internal_initial_point, operator, learning_rate, ansatz, shots, sampler))
...
def Customize_QNSPSA_PRS_blocking_fixrandom(operator, initial_point, learning_rate, ansatz, interation, shots, callback, sampler):
    """
    operator: The pauli operator
    interation: number of interation
    initial_point: the initial point that we will update until we end up with the desired point
