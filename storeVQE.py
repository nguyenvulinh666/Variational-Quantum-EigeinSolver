...
def main():
    ...
    for i in range(number_of_times):
        ...
        #optimize = Customize_QNSPSA_PRS_blocking_MonteCarlo

        optimize = COBYLA
        optimize_run = COBYLA(maxiter=500)

        file_name_parameters = f'{str(optimize.__name__)} - LR {eta} - shots {shots} - interation {interation} - {ansatz_name}({num_qubits},{reps}) - J{J}h{h} - Parameters - {i+2}'
        file_name_energy = f'{str(optimize.__name__)} - LR {eta} - shots {shots} - interation {interation} - {ansatz_name}({num_qubits},{reps}) - J{J}h{h} - Energy - {i+2}'
