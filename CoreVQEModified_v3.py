...
def _grad_pool_workers():
    """Inner-gradient/metric parallelism for one optimizer job.

    Controlled by env var VQE_GRAD_WORKERS:
      unset / 1  -> serial (no pool; no fork overhead) -- the default.
      N > 1      -> a PERSISTENT ProcessPoolExecutor of N workers, created once
                    per job and reused across every iteration.

    run_campaign.py sets this to ~ (cores / concurrent jobs) so that
    (jobs) x (workers) fills the machine without oversubscription (lever #1).
    """
    v = os.environ.get("VQE_GRAD_WORKERS")
    return max(1, int(v)) if v else 1


# One persistent pool per job (process). Created on first use, reused across all
# iterations so workers are NOT re-forked every iteration -- this is what lifts
# core utilization from ~20% toward ~90% on the heavy QN-BDA / gradient sweeps.
...
def _get_pool(workers):
    ...
    if _PERSISTENT_POOL is None or _PERSISTENT_POOL_WORKERS != workers:
        if _PERSISTENT_POOL is not None:
            _PERSISTENT_POOL.shutdown(wait=False)
        _PERSISTENT_POOL = concurrent.futures.ProcessPoolExecutor(max_workers=workers)
        _PERSISTENT_POOL_WORKERS = workers
        atexit.register(_PERSISTENT_POOL.shutdown, wait=False)
    return _PERSISTENT_POOL


def _pool_map(fn, tasks, workers):
    """Map fn over tasks on the persistent pool; recreate once if it broke."""
    global _PERSISTENT_POOL
    try:
        return list(_get_pool(workers).map(fn, tasks))
    except concurrent.futures.process.BrokenProcessPool:
        _PERSISTENT_POOL = None
        return list(_get_pool(workers).map(fn, tasks))


def _parallel_grad(params):
    """Map mini_derivate over params: serial, or on the persistent pool.

    Numerically identical to the original `list(ProcessPoolExecutor().map(...))`;
    only the execution strategy changes (persistent pool instead of per-iteration).
    """
    workers = _grad_pool_workers()
    if workers == 1:
        return [mini_derivate(p) for p in params]
    return _pool_map(mini_derivate, params, workers)


_GATE_TO_PAULI = {'rx': 'X', 'ry': 'Y', 'rz': 'Z'}


def _fubini_element(args):
    """Top-level worker: one block-diagonal Fubini-Study metric element.

    Mirrors the original nested Measure_element_of_Fubini_Study_metric exactly
    (same Pauli strings, same formula); extracted to module scope so it is
    picklable for a ProcessPoolExecutor.
    """
    block, circ_meas, i, j, shots, gl, gm = args
    n = len(block)
    if i != j:
        term1 = ['I'] * n
        term2 = ['I'] * n
        term3 = ['I'] * n
        pi = _GATE_TO_PAULI.get(block[i].operation.name)
        pj = _GATE_TO_PAULI.get(block[j].operation.name)
        if pi:
            term1[i] = pi; term2[i] = pi
        if pj:
            term1[j] = pj; term3[j] = pj
        s1 = ''.join(term1[::-1]); s2 = ''.join(term2[::-1]); s3 = ''.join(term3[::-1])
        val = (Transverse_Ising_Measurement(s1, circ_meas, shots, None)
               - Transverse_Ising_Measurement(s2, circ_meas, shots, None)
               * Transverse_Ising_Measurement(s3, circ_meas, shots, None)) / 4
    else:
        term2 = ['I'] * n
        pi = _GATE_TO_PAULI.get(block[i].operation.name)
        if pi:
            term2[i] = pi
        s2 = ''.join(term2[::-1])
        val = (1 - Transverse_Ising_Measurement(s2, circ_meas, shots, None).real ** 2) / 4
    return gl, gm, val


def _parallel_fubini(super_circuit, ansatz, parameter, shots):
    """Block-diagonal Fubini-Study metric with element-level parallelism.

    Each element is independent; map them over a capped pool (VQE_GRAD_WORKERS)
    so QN-BDA jobs use many cores. Serial (workers=1) is numerically identical
    to the original double-loop implementation.
    """
    ...
    for i in range(len(super_circuit)):
        if super_circuit[i][0].operation.params:
            pp = sum(super_circuit[j].num_parameters for j in range(i))
            internal = QuantumCircuit(ansatz.num_qubits)
            for j in range(i):
                internal = internal.compose(super_circuit[j])
            internal = internal.bind_parameters(
                {theta: parameter[k] for k, theta in enumerate(internal.parameters)})
            nb = super_circuit[i].num_parameters
            for l in range(nb):
                for m in range(l + 1, nb):
                    tasks.append((super_circuit[i], internal, l, m, shots, l + pp, m + pp))
                tasks.append((super_circuit[i], internal, l, l, shots, l + pp, l + pp))
    workers = _grad_pool_workers()
    if workers == 1:
        results = [_fubini_element(t) for t in tasks]
    else:
        results = _pool_map(_fubini_element, tasks, workers)
    for gl, gm, val in results:
        metric[gl][gm] = val
        metric[gm][gl] = val
    return metric
...
def Separate_Circuit_Apart(ansatz):
    ...
    entangling_block = None  # accumulates consecutive non-parametric layers
 
    for layer_circuit in divide_to_layer_circuit:
        instructions = layer_circuit.data
        if len(instructions) == 0:
            continue
 
        operation_names = [instruction.operation.name for instruction in instructions]
        is_barrier_layer = all(name == "barrier" for name in operation_names)
        # A layer is "parametric" (a rotation layer) when its gates carry
        # parameters. DAG layers of these ansatze are homogeneous (all Ry, all
        # Rz, all Cx, or all barrier), matching the original `[0].params` check.
        is_parametric_layer = (not is_barrier_layer) and bool(instructions[0].operation.params)
 
        if is_barrier_layer:
            # Barrier => close any open entangling block.
            if entangling_block is not None:
                super_circuit.append(entangling_block)
                entangling_block = None
            continue
 
        if is_parametric_layer:
            # Each parametric DAG layer is its own block (keeps Ry / Rz apart).
            if entangling_block is not None:
                super_circuit.append(entangling_block)
                entangling_block = None
            super_circuit.append(layer_circuit)
        else:
            # Non-parametric layer (entangling gates): accumulate into one block.
            if entangling_block is None:
                entangling_block = QuantumCircuit(ansatz.num_qubits)
            entangling_block = entangling_block.compose(layer_circuit)
...
def calculate_block_diagonal_fubini_metric(super_circuit, ansatz, parameter, shots, sampler):
    return _parallel_fubini(super_circuit, ansatz, parameter, shots)
...
def Customize_QN_FD(operator, initial_point, learning_rate, ansatz, interation, shots, callback, sampler):
    """
    Args:
        operator: PauliOp
        initial_point: An initial parameter values for the optimizer. 
...
def Customize_QNSPSA_FD_blocking(operator, initial_point, learning_rate, ansatz, interation, step, shots, callback, sampler, previous_fubini_matrix, last_n_steps):
    ...
    for k in range(interation):  
        ...
        params = []
        gradFD = np.zeros(ansatz.num_parameters)

        for i in range(ansatz.num_parameters):
            params.append((i, h, h, internal_initial_point, operator, ansatz, shots, sampler))

        gradFD = _parallel_grad(params)
